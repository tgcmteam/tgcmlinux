...
Name: escritorio-movistar-ecuador
Summary: Ecuador Connection Manager
Version: 8.9
Release: 1
License: GPLv2+
...
find $RPM_BUILD_ROOT/etc/xdg/autostart/*.desktop -name tgcm-*.desktop | grep -v tgcm-ec.desktop | xargs rm -f
find $RPM_BUILD_ROOT/usr/share/applications/ -name tgcm-* | grep -v tgcm-ec | xargs rm -f
find $RPM_BUILD_ROOT/usr/share/doc/tgcm/ -maxdepth 1 -type d | grep -v tgcm/ec | grep -ve "tgcm/$" | xargs rm -fr
